...
{
    public AudioClip audioClipSounds;
...
//	{
		...
		if (media.getSource().contains("sounds")) // Just to keep playing ClipSounds parallel over ClipVoice
		{
			...
			catch (UnsupportedAudioFileException ex) { ui.log("Error: UnsupportedAudioFileException " + Sound.class.getSimpleName() + ".play(..) AudioSystem.getAudioInputStream(" + media.getSource() + " " + ex.getMessage() + " \r\n", true, true, true, true, false); }
			catch (IOException ex) { ui.log("Error: IOException " + Sound.class.getSimpleName() + ".play(..) AudioSystem.getAudioInputStream(" + media.getSource() + " " + ex.getMessage() + " \r\n", true, true, true, true, false); }

			clipSounds = null; try {  clipSounds = AudioSystem.getClip(); } catch (LineUnavailableException ex)
			{ ui.log("Error: LineUnavailableException " + Sound.class.getSimpleName() + ".play(..).AudioSystem.getClip() " + ex.getMessage() + " \r\n", true, true, true, true, false); }

			try { clipSounds.open(audioInputStreamSounds); } 
			catch (LineUnavailableException ex)	{ clipSounds.close(); clipSounds = null; ui.log("Error: LineUnavailableException " + Sound.class.getSimpleName() + ".play(..).clipSounds.open(" + media.getSource() + ") " + ex.getMessage() + " \r\n", true, true, true, false, false); }
			catch (IOException ex)		{ clipSounds.close(); clipSounds = null; ui.log("Error: IOException " + Sound.class.getSimpleName() + ".play(..).clipSounds.open(" + media.getSource() + ") " + ex.getMessage() + " \r\n", true, true, true, false, false); }

			if ( clipSounds != null )
			{
			    clipSounds.start();
			    try { audioInputStreamSounds.close(); } catch (IOException ex) { ui.log("Error: IOException " + Sound.class.getSimpleName() + ".play(..).audioInputStreamSounds.close() " + ex.getMessage() + " \r\n", true, true, true, false, false); }
			}
			else
			{
			    try { audioInputStreamSounds.close(); } catch (IOException ex) { ui.log("Error: IOException " + Sound.class.getSimpleName() + ".play(..).audioInputStreamSounds.close() " + ex.getMessage() + " \r\n", true, true, true, false, false); }			
		...
		else if ( media.getSource().contains("voice") )
		{
			...
			clipVoice = null; try {  clipVoice = AudioSystem.getClip(); } catch (LineUnavailableException ex) { ui.log("Error: LineUnavailableException " + Sound.class.getSimpleName() + ".play(..).AudioSystem.getClip() " + ex.getMessage() + " \r\n", true, true, true, true, false); }

			try { clipVoice.open(audioInputStreamVoice); }
			catch (LineUnavailableException ex) { clipVoice.close(); clipVoice = null; ui.log("Error: LineUnavailableException " + Sound.class.getSimpleName() + ".play(..).clipVoice.open(" + media.getSource() + ") " + ex.getMessage() + " \r\n", true, true, true, false, false); }
			catch (IOException ex)		{ clipVoice.close(); clipVoice = null; ui.log("Error: IOException " + Sound.class.getSimpleName() + ".play(..).clipVoice.open(" + media.getSource() + ") " + ex.getMessage() + " \r\n", true, true, true, false, false); }

			if ( clipVoice != null )
			{
			    clipVoice.start();
			    try { audioInputStreamVoice.close(); } catch (IOException ex) { ui.log("Error: IOException " + Sound.class.getSimpleName() + ".play(..).audioInputStreamVoice.close() " + ex.getMessage() + " \r\n", true, true, true, false, false); }
			}
			else
			{
			    try { audioInputStreamVoice.close(); } catch (IOException ex) { ui.log("Error: IOException " + Sound.class.getSimpleName() + ".play(..).audioInputStreamVoice.close() " + ex.getMessage() + " \r\n", true, true, true, false, false); }			
		...
		else { ui.log("Alert: " + Sound.class.getSimpleName() + ".play(" + media.getSource() + ") not recognized!\r\n", true, true, true, true, false); }
	    }
//	});
...
//	playThread.start();	
    ...
    synchronized public void playJavaFX(UI ui, Media media, int audio_codec) // javafx.scene.media.AudioClip
    {
//	Thread playThread = new Thread(() ->
//	{
	    if (media != null) 
